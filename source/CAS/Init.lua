-- Initializing the init class.
local Init = class()

Init.hooks = {}

Init.hooks.startRound = Hook('startround', function(mode)
  -- Free all the images.
...
Init.hooks.leave = Hook('leave', function(player, reason)
  ...
  for key, value in pairs(Image._images) do
    ...
    if (not value:isVisibleToEveryone() and value:getPlayer() == player)
        or (value._followedPlayer and value._followedPlayer == player) then
...
Init.hooks.collect = Hook('collect', function(player, iid, itemType, ain, a, mode)
...
Init.hooks.objectKill = Hook('objectkill', function(object, player)
