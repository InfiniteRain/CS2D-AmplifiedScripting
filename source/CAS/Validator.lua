        ...
        elseif type(rule.type) == 'table' and rule.type.__IS_CLASS then
            ...
            for _, customRule in pairs(self._customRules) do
                if rule.type == customRule.name then
                    ...
                    if hasError and not (type(rule.value) == 'nil' and rule.optional) then
                        potentialError = {
                            error = true,
                            number = key,
                            message = message
            ...
            if not foundCustomRule then
                error('Rule with a label of "' .. rule.type .. '" wasn\'t found.', 2)
            end
