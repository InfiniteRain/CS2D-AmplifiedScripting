...
-- Constructor.
function HudText:constructor(player, x, y, alignment, ...)
  ...
  if not HudText._alignments[alignment] then
  ...
  local arguments = { ... }
  local textString = (getmetatable(arguments[1]) == Color and '' or tostring(Color(255, 220, 0)))
  ...
  for key, value in pairs(arguments) do
    ...
    elseif type(value) == 'string' then
      ...
      error('Argument #' .. key + 4 .. ' should either be an instance of the Color class or a string.', 2)
    end
  end

  -- Checking if the limit for hud texts was reached.
  local hudTexts = 0
  for key, value in pairs(HudText._hudTexts[player._id]) do
    hudTexts = hudTexts + 1
  end
  if hudTexts >= 50 then
  ...
  for id = 0, 49 do
    if not HudText._hudTexts[player:getSlotID()][id] then
      self._id = id
      HudText._hudTexts[player:getSlotID()][id] = self
      break
    end
  end
  ...
  self._freed = false
  self._color = (getmetatable(arguments[1]) == Color and arguments[1] or Color(255, 220, 0))
  ...
  self._tMove = {
    ...
    timer = Timer(function(self) -- The timer function, which updates the fields upon
  ...
  self._tColor = {
    ...
    timerStartingMS = 0,
    timerLifetime = 0,
    timer = Timer(function(self)
      self._tColor.active = false
      self._color = Color(self._tColor.finalColor[1], self._tColor.finalColor[2], self._tColor.finalColor[3])
    end)
  }
  ...
  -- Showing the hudtext.
  Console.parse('hudtxt2', self._player:getSlotID(), self._id, textString, x, y, HudText._alignments[alignment])
end

function HudText:destructor()
...
-- Frees the hud text.
function HudText:free()
  if self._freed then
    error('This hud text was already freed. It\'s better if you dispose of this instance.', 2)
  end

  -- Frees the hud text.
  Console.parse('hudtxt2', self._player:getSlotID(), self._id)
  self._freed = true

  -- Finds it in the '_hudTexts' table of the player and removes it.
  for key, value in pairs(HudText._hudTexts[self._player:getSlotID()]) do
    if value == self then
      HudText._hudTexts[self._player:getSlotID()][key] = nil
      return
    end
  end
...
-- Tween move.
function HudText:tweenMove(time, x, y)
  ...
  Console.parse('hudtxtmove', self._player:getSlotID(), self._id, time, x, y)
...
-- Tween color.
function HudText:tweenColor(time, color)
  if type(time) ~= 'number' then
    error('Passed "time" parameter is not valid. Number expected, ' .. type(time) .. ' passed.', 2)
  ...
  Console.parse('hudtxtcolorfade', self._player:getSlotID(), self._id, time, color:getRGB())
...
-- Tween alpha.
function HudText:tweenAlpha(time, alpha)
  if type(time) ~= 'number' then
    error('Passed "time" parameter is not valid. Number expected, ' .. type(time) .. ' passed.', 2)
  ...
  Console.parse('hudtxtalphafade', self._player:getSlotID(), self._id, time, alpha)
...
-- Stop tween move.
function HudText:stopTweenMove()
  if self._freed then
    error('This hud text was already freed. It\'s better if you dispose of this instance.', 2)
  end
  ...
  Console.parse('hudtxtmove', self._player:getSlotID(), self._id, 0, nx, ny)
  self._x = nx
  self._y = ny
...
-- Stop tween color.
function HudText:stopTweenColor()
  if self._freed then
    error('This hud text was already freed. It\'s better if you dispose of this instance.', 2)
  end
  ...
  Console.parse('hudtxtcolorfade', self._player:getSlotID(), self._id, 0, color:getRGB())
  self._color = color
...
-- Stop tween alpha.
function HudText:stopTweenAlpha()
  if self._freed then
    error('This hud text was already freed. It\'s better if you dispose of this instance.', 2)
  end
  ...
  Console.parse('hudtxtalphafade', self._player:getSlotID(), self._id, 0, self:getAlpha())
...
function HudText:setText(...)
  ...
  self._text = textString
  Console.parse('hudtxt2', self._player:getSlotID(), self._id, textString, self._x, self._y, 0)

  return self
end

-- Sets the position of the hudText.
function HudText:setPosition(x, y)
  ...
  self._x = x
  self._y = y
  Console.parse('hudtxtmove', self._player:getSlotID(), self._id, 0, x, y)

  return self
end

-- Sets the position of the hudText on the X axis.
function HudText:setX(x)
  if type(x) ~= 'number' then
    error('Passed "x" parameter is not valid. Number expected, ' .. type(x) .. ' passed.', 2)
  end
...
function HudText:setColor(color)
  ...
  self._color = color
  Console.parse('hudtxtcolorfade', self._player, self._id, 0, color:getRGB())

  return self
end

-- Sets the alpha of the hudText.
function HudText:setAlpha(alpha)
  ...
  self._alpha = alpha
  Console.parse('hudtxtalphafade', self._player:getSlotID(), self._id, 0, alpha)
...
function HudText:getColor()
  ...
  if self._tColor.active then
    ...
    local multiplier = (((os.clock() * 1000) - timerStarted) / timerNeeded)
    return Color(math.floor(starting[1] + (finishing[1] - starting[1]) * multiplier),
...
HudText._hudTexts = {} -- Table which holds all the hud texts of the players.
for id = 1, 32 do
  HudText._hudTexts[id] = setmetatable({}, { __mode = 'kv' }) -- Giving each player its own hudText table and making
  -- the table weak.
end
